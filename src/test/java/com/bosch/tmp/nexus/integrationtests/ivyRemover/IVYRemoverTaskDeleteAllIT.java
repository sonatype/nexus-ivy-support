...
* @author wa21190
*/
public class IVYRemoverTaskDeleteAllIT 
 extends AbstractNexusIntegrationTest
{
        ...
        throws ComponentLookupException
    {
        ...
        this.repoUtil = new RepositoryMessageUtil( this, XStreamFactory.getXmlXStream()
                                                       , MediaType.APPLICATION_XML ); 
    }
    
    @Test
    public void testSnapRepoDeployFilesRunTaskZero()
        throws IOException, Exception
    {
        ...
        //update indexes of snapshot repository
        RepositoryMessageUtil.updateIndexes(snapRepoId);
        
        TaskScheduleUtil.waitForAllTasksToStop();
        
        //set the parameters of SnapshotRemoverTask, nothing should be left over in the repository
        ...
        //firstly assert that the artifact "auth-mod" verion "1.0-100" is deleted
        Gav gav = GavUtil.newGav( "ivyRemover", "auth-mod", "1.0-100" );

        List<NexusArtifact> result = getSearchMessageUtil()
                                    .searchForGav( gav, snapRepoId );
        
        Assert.assertEquals( result.size(), 0, "Results: \n" 
                                + XStreamFactory.getXmlXStream().toXML( result ) );
        
        //secondly assert that the artifact "auth-mod" version "1.0-101" is deleted
        gav = GavUtil.newGav( "ivyRemover", "auth-mod", "1.0-101" );

        result = getSearchMessageUtil().searchForGav( gav, snapRepoId );
        Assert.assertEquals( result.size(), 0, "Results: \n" 
                                + XStreamFactory.getXmlXStream().toXML( result ) );

        //thirdly assert that the artifact "auth-mod" version 1.0-102" is deleted
        ...
        //fourthly assert that the artifact "auth-mod" version 1.0-103 is deleted
        ...
        //fifthly assert that the artifact "auth-api" version 0.8-100 is deleted
        gav = GavUtil.newGav( "ivyRemover", "auth-api", "0.8-100" );

        result = getSearchMessageUtil().searchForGav( gav, snapRepoId );
        Assert.assertEquals( result.size(), 0, "Results: \n" 
                                + XStreamFactory.getXmlXStream().toXML( result ) );
        
        //sixthly assert that the artifact "auth-api" version "0.8-101" is deleted
        gav = GavUtil.newGav( "ivyRemover", "auth-api", "0.8-101" );

        result = getSearchMessageUtil().searchForGav( gav, snapRepoId );
        Assert.assertEquals( result.size(), 0, "Results: \n" 
                                + XStreamFactory.getXmlXStream().toXML( result ) );
        
        //seventhly assert that the artifact "auth-api" version "0.8-102" is deleted
        gav = GavUtil.newGav( "ivyRemover", "auth-api", "0.8-102" );

        result = getSearchMessageUtil().searchForGav( gav, snapRepoId );
        Assert.assertEquals( result.size(), 0, "Results: \n" 
                                + XStreamFactory.getXmlXStream().toXML( result ) );
        
        //eighthly assert that the artifact "auth-api" version "0.8-103" is deleted
        ...
        //ninthly assert that there at least no artifacts left over in the group folder of the snapshot repository       
