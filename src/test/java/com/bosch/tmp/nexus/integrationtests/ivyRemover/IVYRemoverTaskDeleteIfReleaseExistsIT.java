...
* @author wa21190
*/
public class IVYRemoverTaskDeleteIfReleaseExistsIT 
 extends AbstractNexusIntegrationTest
{
    ...
    {   
        ...
        getDeployUtils().deployUsingGavWithRest( repoId, gav, 
                                    getTestFile( "simple-artifact.jar" ) );
        ...
        //thirdly assert that there is at least alltogether one release artifact in the release repository
        result = getSearchMessageUtil().searchFor( Collections
                                .singletonMap( "q", "ivyRemover" ), repoId );
        Assert.assertEquals( result.size(), 1, "Results: \n" 
                                + XStreamFactory.getXmlXStream().toXML( result ) );
    ...
    @Test
    public void testSnapRepoDeployFilesRunTaskDeleteIFRelease()
        throws IOException, Exception
    {
        ...
        //firstly assert that there is the artifact "auth-api" version "0.8-102" left over with ".source.jar" and ".jar" 
        ...
        //secondly assert that there is the artifact "auth-api" version "0.8-103" left over with ".source.jar" and ".jar"
        ...
        //fifthly assert that there are at least 2 of 8 snapshots with 2 jar files in the whole test group, 6 removed
        result = getSearchMessageUtil()
                .searchFor( Collections.singletonMap( "q", "ivyRemover" ), snapRepoId );
        Assert.assertEquals( result.size(), 4, "Results: \n" 
                                + XStreamFactory.getXmlXStream().toXML( result ) );
